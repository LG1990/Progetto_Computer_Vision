...
int myRoutine()
{
	...
	for(;;)
	{		
		if(frame_number%(int)(cap.get(CV_CAP_PROP_FPS)*2)==0 || frame_number==1)	//reinitialize if we are at the first frame or every 2 sec; frmae_rate->CV_CAP_PROP_FPS
		{ needToInit = true; }
		else 
		{ needToInit = false; }
...
void myHotspotDetector(vector<Point2f> myPoints, int myCapHeight, int myCapWidth)
{
	...
	{
		cv::circle(my2dMap, myPoints.at(i), 1, Scalar(255), -1, 8);
	}
	...
	if(!myHSPoints.empty())
	{
		for (int v = 0; v < myHSPoints.size()-1; v++)
		{
			if(myHSPoints[v].z==0)
			{
				for (int i = 0; i <  myHSPoints.size(); i++)
				{
					if(abs(myHSPoints[v].x-myHSPoints[i].x) < 10 && i!=v)
					{ index.push_back(i); }
				}
				for (int i = 0; i < index.size(); i++)
				{
					if(myHSPoints[v].z==0){ myHSPoints[v].z = zone_number + 1; }
					if(abs(myHSPoints[v].y-myHSPoints[index[i]].y) < 10)
					{ 
...
/*
	if(!myHSPoints.empty())
	{
		for (int v = 0; v < myHSPoints.size()-1; v++)
		{
			if(norm(myHSPoints[v].x - myHSPoints[v+1].x) <=10 && norm(myHSPoints[v].y - myHSPoints[v+1].y) <=10)
			{
				myHSPoints[v+1].z = myHSPoints[v].z;
			}
			else if(myHSPoints[v+1].z==0)
			{
				for (int i = 0; i < myHSPoints.size(); i++)
				{
					if(norm(myHSPoints[i].x - myHSPoints[v+1].x) <=10 && norm(myHSPoints[i].y - myHSPoints[v+1].y) <=10 && myHSPoints[i].z!=0)
					{ myHSPoints[v+1].z = myHSPoints[i].z; }
				}
			}
			else
			{
				zone_number++;
				myHSPoints[v+1].z = zone_number;
			}
		}
	}
	*/
	...
	if(!myHSPoints.empty())
	{
		for (int i = 0; i < myHSPoints.size(); i++)
		{
			cv::circle(my2dMap, Point(myHSPoints.at(i).x, myHSPoints.at(i).y), 3, Scalar(0,0,255), -1, 0);
		}
	}

	imshow("Hotspot detection", my2dMap);
